...
$defaultXStartup = qq(#!/usr/bin/env bash
module add xfce 2>/dev/null 1>&2
...
chomp $hostname;
my $port = $displayNumber;
my $shifted_port = $port + 5900;
my $help_string = qq"
...
If you use remmina, set the following options in order to connect:
 ...
 Under Basic:
  ...
  Username: $ENV{USER}
  Password: <The VNC password you set> <changeable with 'vncpasswd'>
...
If you use chicken of the vnc
 Under 'Servers':
  Host: $ENV{USER}\@${hostname}:${shifted_port}
...
If you use an ssh tunnel + some other client
 1.  Perform the following command on your local host:
  ssh -L ${shifted_port}:localhost:${shifted_port} $ENV{USER}@${hostname}
 2.  Perform the following command on your local host:
  vncviewer localhost:${port}
I actually prefer vncviewer -fullscreen but that can be a pain if your
keyboard is non-standard.

One more note:
The default session installed for this vnc is xfce,
the tab key doesn't work in a 'normal' way by default.
In order to fix this:
 1. Go to the application menu and click settings -> window manager
 2. Go to the keyboard shortcuts thing and find a shortcut with 'super<tab>'
   Its about 1/3 down the screen.
 3. Delete that shortcut; tab should work immediately.
";
